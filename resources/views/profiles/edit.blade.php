<x-app>
    <form action="{{route('profile', $user->username)}}" method="POST" enctype="multipart/form-data">
    @csrf
        @method('PATCH')
        ...
        <div class="mb-6">
            ...
            <input class="border border-gray-400 p-2 w-full" type="text" value="{{$user->username}}" name="username" id="username" required>
            @error('username')
                <p class="text-red-500 text-xs mt-2">{{$message}}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input class="border border-gray-400 p-2 w-full" type="email" value="{{$user->email}}" name="email" id="email" required>
            @error('email')
                <p class="text-red-500 text-xs mt-2">{{$message}}</p>
            @enderror
        </div>
